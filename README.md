...
Craft settings window contains shortcuts to most used moderation and tuning parameters of the craft, as well as provides basic preset functionality. Presets are saved in `Global_settings.cfg/settings_wnd/profile`s section.
...
Cruise Flight (CF) is high-level autopilot, designed for travel automation. Just like MD, CF is inherently-linear, so only relatively small angles of attack are allowed. All AoA moderations are forcefully turned on during its operation.
...
Short GUI description:
* _Level_ - simple levelling regime. Upon activation, CF will save surface-relative inclination of velocity and will follow it. If altitude is not set, will keep vertical speed at zero.
...
* _strength mult_ - default value 0.75. Will be multiplied in the runtime on Director controller's strength to restrain maneovers. Tune to achieve slower or faster behaviour.
...
* _max climb angle_ - default value 30 degrees. Global limit on climb and drop maneover velocity pitch. Will sometimes be exceeded, it's okay.
...
Short GUI description (consult source code for more details and insight):
* Three sections for three craft principal axes, each contains:
  ...
  * _ang acc_ - angular acceleration, produced by numerical differentiation.
...
* two vectors on engine torque linear estimations. They are used to address gimbaling capabilities of a craft.
...
Short GUI description:
* _strength_ - default value 0.95. Measure of aggressiveness of acceleration output of MD. Precise control multiplies output acceleration by the factor of 0.4. Serialized per vessel design.
...
* _max angular v_ - estimate on current maneover maximum angular velocity.
* _stop time roll_ - estimate on 90-degrees bank maneover stop time.
...
* _roll cubic relax frame_ - default value 10.0. Relaxation frame for cubic descent phase. Magic number.
...
* _snapping boundary_ - default value 3 degrees. On low bank error modes we will transition from cubic relaxation to proportional relaxation (like in roll controller wing leveller code).
...
When navball is in surface mode, controller is dealing with surface-oriented reference frame. Zero input will keep zero angular velocity relative to the ground - useful on planes. In orbit navball mode inertial reference frame will be used - useful for spacecrafts. Precision mode (CAPS LOCK) multiplies input by the factor of 0.33 (_precision mode factor_ option in global_settings.txt) to provide more precise control, or to aid with control on high physical warp regimes. To ignore precision mode, unset _watch precision mode_ toggle in respected ang vel controllers.

Short GUI description:

* _Auto trim_ button - turn on off you want control trim to preserve after controller shutdown. Off by default.
...
* _max\min aoa v_ - equilibrium angular velocities for set by user AoA limit.
...
* _quadr Kp_ - default value - 0.3. Controller uses parabolic descent model of angular velocity to its desired value. Those descent parameters are governed by this coefficient. Larger values may cause overshoot from wrong control surface lag handling. Lower values will slow down control. Magic number.
* _kacc quadr_ - parabolic steepness of control, governed by control utilities authority and craft characteristics. Should be positive.
* _kacc smoothing_ - default value - 10.0. Filter gain for slow and smooth "kacc quadr" evolution. Magic number.
* _relaxation k_ - default value - 1.0. Controller uses relaxed linear descent on very small velocity error regimes. This coefficient governs relaxation frame size.
...
* _transit max v_ - very rough, but safe estimation of maximum non-overshooting velocity in transit maneover (from 0.0 AoA to maximum AoA).
* _res max\min aoa_ - AoA limits, that will actually govern controller in the current frame. Are chose as the most strict ones from previously listed.
...
* _Moderate G-force_ button - toggle G-force moderation. Moderates centrifugal acceleration of trajectory, not the full one, so G-meeter on navball will sometimes exceed maximum value (it is a correct behaviour).
...
Model-reference controller, that perform roll angular velocity control and wing levelling. Input: [-1, 1] user input or desired angular velocity. Output: desired angular acceleration, passed to angular acceleration controller.
...
* _Wing leveler_ - toggle to level wings automatically, if craft is close to zero bank angle. Zero angle is not horizontal one, but the normal one to the trajectory plane - good for levelling on non-zero pitch while yawing.
...
* _angle btw hor sin_ - sinus of the horizontal angle.
* _snapping Kp_ - snapping speed gain. Default value - 0.25. Larger values seem to be too aggressive, too large oscillate.
...
* _break spd margin_ - when surface speed is exceeding desired by this margin, brakes will be used. On ground breaks are used without margin.
